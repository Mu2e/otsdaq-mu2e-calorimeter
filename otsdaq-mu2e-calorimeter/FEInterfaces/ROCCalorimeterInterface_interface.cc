  ...
  : ROCPolarFireCoreInterface(rocUID, theXDAQContextConfigTree, theConfigurationPath)
{
  ...
  registerFEMacroFunction("Evaluate BlockWrite Error Rate",
			  ...
			  std::vector<std::string>{"Number of test loops, Default := 900]", "Number of 16-bits words in a block transfer, Default := 500]"}, //inputs parameters
			  std::vector<std::string>{"Status"}, //output parameters
			  1);  // requiredUserPermissions
...
void ROCCalorimeterInterface::ROCSlowControl(__ARGS__)
{
  ...
  os << "Word " << 0 << ":  VTRX_REG_Control: 0x" << std::hex << data3[0] << " " << __E__; 
  os << "Word " << 1 << ":  VTRX_REG_ModCurrent: 0x" << std::hex << data3[1]<< " " << __E__; 
  os << "Word " << 2 << ":  VTRX_REG_BiasCurrent: 0x" << std::hex << data3[2] << " " << __E__; 
  os << "Word " << 3 << ":  VTRX_REG_PreEmphasis: 0x" <<  std::hex  << data3[3] << " " << __E__; 
  os << "Word " << 4 << ":  VTRX_REG_ModMask: 0x" << std::hex << data3[4]<< " " << __E__; 
  os << "Word " << 5 << ":  VTRX_REG_BiasMask: 0x" << std::hex << data3[5] << " " << __E__; 
  os << "Word " << 6 << ":  VTRX_REG_PreDriver: 0x" << std::hex << data3[6]   << " " << __E__; 
  */
...
void ROCCalorimeterInterface::EvaluateBlockWriteErrorRate(__ARGS__)
{
  ...
  for (uint16_t iloop = 0; iloop < nloops; iloop++) {
    ...
    while(datain.size() > blockLen) datain.pop_back(); //maybe not needed anymnore
    ...
    while(icomp<dataout.size()){
      ...
      //else if(icomp == 0) os << iloop << " loop, first counter is " << datain[icomp]  <<   __E__; 
      icomp++;
    }
                                           ...
                                           bool incrementAddress)
{
  ...
  {
    __FE_SS__ << "ROC block read failed, expecting " << u-4
    ...
    for(int ichan = 0; ichan<20; ichan++){
      ...
      for (auto pin : _pin_diode_list){
        if (boardID*100+chindex == pin){
          thr2set = thr;
          __COUT_INFO__ << "Board " << boardID << " Channel " << chindex << " detected as pin diode. Ignoring requested offset. Threshold: " << thr2set << __E__;
          break;
        }
      }
