    ...
    skipAfterN_(config().skipAfterN())
{
  ...
  if (filetemp->Get(hname)) spline = (TSpline3 *)filetemp->Get(hname);
  else {std::cout << "No template.." << std::endl;}  

  f_spline=new TF1("f_spline",[this](double *x, double *par) { return par[0]*this->spline->Eval(x[0]-par[1])+par[2];},0.,19.,3);
...
void mu2e::CaloiercAnalyzer::analyze(art::Event const& event){
  ...
  if (verbosity_ > 0){
    ...
    for (auto pair : digiMap){
      ...
      std::cout<<"SiPM ID "<<thisID<<" [dtc: "<<thisID/120<<", roc: "<<(thisID%120)/20<<" ("<<(thisID/20)%6<<"), chan: "<<thisID%20<<"] ("<<pair.second.size()<<" hits) : ";
  ...
  if (uset0_){ //Use t0 instead of fitting
    ...
    for (auto pair : digiMap){
      for (uint ihit=0; ihit<pair.second.size(); ihit++){
        if (skipAfterN_ > 0 && int(ihit) >= skipAfterN_) continue; //stop fitting if surpassed N hits
        uint idx = pair.second[ihit]; //idx is the index in the original caloDigis vector
        gadc->Set(0);
        //std::cout<<"Fitting sipm "<<pair.first<<", hit "<<ihit<<", (idx: "<<idx<<")\nWaveform: ";
        ...
        if (fitStatus >= 0){
          //std::cout<<"fitStatus: "<<fitStatus<<", ped: "<<f_spline->GetParameter(2)<<", time: "<<f_spline->GetParameter(1)<<", scale: "<<f_spline->GetParameter(0)<<"\n";
          //std::cout<<"chi2: "<<f_spline->GetChisquare()<<", ndf: "<<f_spline->GetNDF()<<", chi2/ndf: "<<f_spline->GetChisquare()/f_spline->GetNDF()<<"\n";
          timeMap[pair.first].push_back(5.*(caloDigis[idx].t0()+f_spline->GetParameter(1)));
          chi2Map[pair.first].push_back(f_spline->GetChisquare());
          chi2rMap[pair.first].push_back(f_spline->GetChisquare()/f_spline->GetNDF());
        } else { //if bad fit, don't fill at all
          std::cout<<"Bad fit status: "<<fitStatus<<" for sipmid "<<pair.first<<" hit "<<idx<<"\n";
          total_badfits++;
        }
      }
    }
  ...
  for (auto pair : timeMap){
    ...
    if (thisDTCROC == previous_DTCROC) continue; //Skip if we are on the same board as previous
    ...
    for (auto nextpair : timeMap){
      ...
      if (thisDTCROC == nextDTCROC){ //We found a different channel in the same board
        ...
        if (map_h1_dt_sameboard.find(idpair) == map_h1_dt_sameboard.end()){//This hist doesn't exist yet
          ...
          TString htitle = Form("[SAME BOARD] dt between chan %d and %d (DTC: %d, Board: %d, hit 0)",nextChan,thisChan,thisDTC,thisROC);
          ...
          TString gtitle = Form("[SAME BOARD] dt between chan %d and %d (DTC: %d, Board: %d, hit 0)",nextChan,thisChan,thisDTC,thisROC);
        ...
        if (abs(dt)>5000) continue; //They are not the same hit, discard
        //std::cout<<"Same board, filling evt "<<this_eventNumber<<", thisID: "<<thisID<<", nextID: "<<nextID<<", dt = "<<dt<<"\n";
  ...
  for (int chan=0; chan<20; chan++){
    ...
    for (auto pair : timeMap){
      ...
      int thisROC = thisDTCROC%6;
      int thisChan = thisID%20;
      //if (chan==1){std::cout<<"thisChan: "<<thisChan<<", thisID: "<<thisID<<", refID: "<<refID<<"\n";}
      if (thisChan == chan){ //Found the channel we want to plot
        if (refID < 0){//It's the first one, don't do anything
          ...
          //if (chan==1) {std::cout<<"filling event "<<this_eventNumber<<", refID: "<<refID<<", thisID: "<<thisID<<" -------------------------------------\n";}
          ...
          if (map_h1_dt_diffboard.find(idpair) == map_h1_dt_diffboard.end()){//This hist doesn't exist yet
            ...
            TString htitle = Form("[DIFFERENT BOARD] dt between (DTC: %d, Board: %d) and (DTC: %d, Board: %d) [chan %d, hit 0]",thisDTC,thisROC,refDTC,refROC,thisChan);
            ...
            TString gtitle = Form("[DIFFERENT BOARD] dt between (DTC: %d, Board: %d) and (DTC: %d, Board: %d) [chan %d, hit 0]",thisDTC,thisROC,refDTC,refROC,thisChan);
