...
bool mu2e::CaloFilterDQM::filter(art::Event& event) {
	...
	if(metricMan != nullptr) {
		...
		for (auto fail : failure_type_counter) {
			metricMan->sendMetric("CaloFilterDQM.FailedHits_" + caloDAQUtil_.getCaloHitErrorName(fail.first), int(fail.second), "Failed hits, by failure type", metrics_reporting_level_, artdaq::MetricMode::LastPoint);
		}
		...
		for (const auto& dtcLinkFail : failures_per_dtc_link) {
			...
			for (const auto& rocFail : dtcLinkFail.second) {
				...
				metricMan->sendMetric("CaloFilterDQM.FailedHits_DTC" + std::to_string(dtcID) + "_Link" + std::to_string(rocID), int(nFails), "Failed hits, by DTC link and ROC ID", metrics_reporting_level_, artdaq::MetricMode::LastPoint);
			}
		}
		for (const auto& dtcLinkGood : goodHits_per_dtc_link) {
			...
			for (const auto& rocGood : dtcLinkGood.second) {
				...
				metricMan->sendMetric("CaloFilterDQM.GoodHits_DTC" + std::to_string(dtcID) + "_Link" + std::to_string(rocID), int(nGoods), "Good hits, by DTC link and ROC ID", metrics_reporting_level_, artdaq::MetricMode::LastPoint);
			}
		}
		// Per-event
		...
		if (verbosity_ > 1) {
			std::cout << "Metrics for event " << eventNumber << ":"
				<< " nCaloDTCs=" << nCaloDTCs  
				<< ", nEventCaloHits=" << nEventCaloHits
			    << ", nTotalBadHits=" << nTotalBadHits 
				<< std::endl;
